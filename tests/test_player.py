...
class PlayerTest(unittest.TestCase):
    def test_stores_name_and_hand(self):
        hand = Hand()
        player = Player(name = "Fadlullah", hand = hand)
        self.assertEqual(player.name, "Fadlullah")
        self.assertEqual(player.hand, hand)

    def test_figures_out_own_best_hand(self):
        mock_hand = MagicMock()
        mock_hand.best_rank.return_value = "Straight Flush"
        player = Player(name = "Fadlullah", hand = mock_hand)
    ...
    def test_passes_new_cards_to_hand(self):
        mock_hand = MagicMock()
        player = Player(name = "Jide", hand = mock_hand)
    ...
    def test_decides_to_continue_or_drop_out_of_game(self):
        player = Player(name = "Felix", hand = Hand())
    ...
    def test_is_sorted_by_best_hand(self):
        ...
        player1 = Player(name = "Joven", hand = mock_hand1)
        player2 = Player(name = "FOB", hand  = mock_hand2)
